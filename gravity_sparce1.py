...
def simulation(psi0, V, Dx, Dy, Nt=200, mu=1, skip=5):
    ...
    density = [np.abs(psi0) ** 2]  # Stockage de density0 dans la liste
...
psi0 /= np.sqrt(np.conj(psi0) @ psi0)
...
cax3 = ax[1].plot_surface(X, Y, Energy0 + Norm * density[0], cmap='viridis', edgecolor='none')
...
def slider_update(val):
    ...
    img2 = ax[1].plot_surface(X, Y, Energy0 + Norm * Z, cmap='viridis', edgecolor='none')
